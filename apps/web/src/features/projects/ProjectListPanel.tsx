...
export function ProjectListPanel({ projects, total, loading, error, onOpen, onDelete, onClose }: { readonly projects: readonly ProjectSummaryDto[]; readonly total: number; readonly loading: boolean; readonly error?: string; onOpen(id: string): void; onDelete(project: ProjectSummaryDto): void; onClose(): void }) {
  ...
  return (
    <section className="transient-panel projects-panel" aria-labelledby="projects-heading" aria-busy={loading}>
      ...
      {loading ? <p role="status">Loading projects…</p> : error ? <p className="panel-error" role="alert">{error}</p> : projects.length === 0 ? <p>You have no saved projects yet.</p> : (
        <>{total > projects.length ? <p className="panel-note">Showing the 20 most recently updated projects.</p> : null}<ul className="project-list">{projects.map((project) => <li key={project.id}>
