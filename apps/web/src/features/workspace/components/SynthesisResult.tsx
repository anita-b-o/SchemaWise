...
export function SynthesisResult({ result, snapshot, onViewDiagram }: SynthesisResultProps) {
  ...
  return (
    <section className="transformation-result" aria-labelledby="synthesis-result-heading">
      ...
      <p className="transformation-properties"><strong>Dependency preserving</strong><span aria-hidden="true"> · </span><strong>Lossless join</strong></p>
      ...
      <details className="result-disclosure educational-disclosure result-explanation">
        <summary>Explain synthesis</summary>
        <div className="educational-disclosure__content">
          <p className="explanation-label"><strong>Why these relations</strong></p>
          <p><Content tokens={explanation.summary} lookup={lookup} /></p>
          <ul className="relation-provenance">
            {result.relations.map((relation, index) => <li key={`${relation.attributes.join(":")}:${index}`}>Relation {index + 1}: {synthesizedRelationSourceLabel(relation.source)}</li>)}
          </ul>
