import type { AnalysisResponseDto, BcnfDecompositionResponseDto, ClosureResponseDto, DependencyPreservationResponseDto, FunctionalDependencyDto, SchemaInputDto, ThirdNormalFormSynthesisResponseDto } from "../../api/schemawise-contracts";
...
export interface ClosureState extends RequestState<ClosureResponseDto> { readonly inputRevision?: number; readonly inputSnapshot?: SchemaInputDto; readonly selectedAttributes?: readonly string[]; readonly outOfDate: boolean; }
export interface WorkspaceState { readonly draft: SchemaDraft; readonly revision: number; readonly analysis: AnalysisState; readonly closure: ClosureState; readonly synthesis: TransformationState<ThirdNormalFormSynthesisResponseDto>; readonly bcnf: TransformationState<BcnfDecompositionResponseDto>; readonly dependencyPreservation: TransformationState<DependencyPreservationResponseDto>; readonly requests: { readonly analysis: RequestState<unknown>; readonly closure: RequestState<unknown>; readonly synthesis: RequestState<unknown>; readonly bcnf: RequestState<unknown>; readonly dependencyPreservation: RequestState<unknown>; }; }
...
export function createInitialWorkspaceState(): WorkspaceState { return { draft: { relationName: "", attributes: [], functionalDependencies: [] }, revision: 0, analysis: { status: "idle", outOfDate: false }, closure: { status: "idle", outOfDate: false }, synthesis: { status: "idle" }, bcnf: { status: "idle" }, dependencyPreservation: { status: "idle" }, requests: { analysis: { status: "idle" }, closure: { status: "idle" }, synthesis: { status: "idle" }, bcnf: { status: "idle" }, dependencyPreservation: { status: "idle" } } }; }
...
function mutate(state: WorkspaceState, draft: SchemaDraft): WorkspaceState { return { ...state, draft, revision: state.revision + 1, analysis: { ...state.analysis, outOfDate: state.analysis.status === "success" || state.analysis.inputSnapshot !== undefined }, closure: { ...state.closure, outOfDate: state.closure.inputSnapshot !== undefined } }; }
...
export function workspaceReducer(state: WorkspaceState, action: WorkspaceAction): WorkspaceState {
  switch (action.type) {
    ...
    case "closureRequestStart": return { ...state, closure: { ...state.closure, status: "loading", requestId: action.requestId, error: undefined, inputRevision: action.inputRevision, inputSnapshot: action.inputSnapshot, selectedAttributes: [...action.selectedAttributes], outOfDate: false }, requests: { ...state.requests, closure: { status: "loading", requestId: action.requestId } } };
    case "closureSuccess": {
      if (state.closure.requestId !== action.requestId || state.closure.status !== "loading" || state.closure.inputRevision === undefined) return state;
      return { ...state, closure: { ...state.closure, status: "success", data: action.data, outOfDate: state.revision !== state.closure.inputRevision }, requests: { ...state.requests, closure: { status: "success", requestId: action.requestId, data: action.data } } };
    }
    case "closureError": {
      if (state.closure.requestId !== action.requestId) return state;
      return { ...state, closure: { ...state.closure, status: "error", error: action.error }, requests: { ...state.requests, closure: { status: "error", requestId: action.requestId, error: action.error } } };
    }
    case "closureAborted": {
      if (state.closure.requestId !== action.requestId) return state;
      const { requestId: _requestId, ...closure } = state.closure;
      return { ...state, closure: { ...closure, status: state.closure.data ? "success" : "idle" }, requests: { ...state.requests, closure: { status: "idle" } } };
    }
